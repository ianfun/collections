...
proc jit_bf(): int32 = 
  while true:
    if cur == bf.len:
      return
    case bf[cur]:
    of '+': emit(openarray([0x41'u8, 0xfe'u8, 0x04'u8, 0x24'u8]))           # incb   (%r12)
    of '-': emit(openarray([0x41'u8, 0xfe'u8, 0x0c'u8, 0x24'u8]))           # decb   (%r12)
    of '>': emit(openarray([0x49'u8, 0xff'u8, 0xc4'u8]))                    # inc    %r12
    of '<': emit(openarray([0x49'u8, 0xff'u8, 0xcc'u8]))                    # dec    %r12
    of '.': emit(openarray([0x41'u8, 0x0f'u8, 0xb6'u8, 0x0c'u8, 0x24'u8,    # movzbl (%r12),%ecx
                            0x41'u8, 0xff'u8, 0xd6'u8]))                    # callq  *%r12 (putchar)
    of ',': emit(openarray([0x41'u8, 0xff'u8, 0xd5'u8,                      # callq  *%r13 (getchar)
